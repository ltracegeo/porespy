...
def snow_partitioning_parallel(im,
    ...
    if largest_radius > min_chunk_dim / 2:
        logger.warning(
            ...
            "This can lead to **incorrect segmentation results** because a "
            "feature's center may lie outside the neighborhood considered for "
            "overlap calculation. It is strongly recommended to "
            "**decrease** the number of divisions ('divs') to make chunks "
            "larger."
        )
...
def _trim_variable_overlap(chunk, overlaps):
    ...
    for i in range(chunk.ndim):
        ...
        start -= np.array(overlaps[i, 0] != 0, np.int)
        end += np.array(overlaps[i, 1] != 0, np.int)
        s_.append(slice(start, end))
